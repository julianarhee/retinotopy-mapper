...
deg_per_frame = rotationRate * 360. * 1./fps
...
while clock.getTime()<=duration: #frame_counter < frames_per_cycle*num_seq_reps: #endPoint - posLinear <= dist: #frame_counter <= frames_per_cycle*num_seq_reps: 
    ...
    stim.ori = frame_counter * deg_per_frame
    stim.draw()
    win.flip()

    print frame_counter, stim.ori
    ...
    if acquire_images:
        ...
        if save_images:
            fdict = dict()
            fdict['im'] = im_array
            fdict['sq_size'] = sq_size
            fdict['rotationRate'] = rotationRate
            fdict['flashPeriod'] = flashPeriod
            fdict['frame'] = frame_counter #nframes
            #print 'frame #....', frame_counter
            fdict['time'] = datetime.now().strftime(FORMAT)
            fdict['stimOri'] = stim.ori
