...
# parser.add_option('--freq', action="store", dest="freq", default=0.13, help="temporal frequency of larger circle, Hz")
...
while True:
    ...
    exptID = str(subID) + '_circle_' + str_cyc.replace('.', '') + 'Hz'
    ...
    num_cycles = num_cond_reps #{'0': 0, '1': num_cond_reps}
    ...
    while clock.getTime()<=duration*num_cycles: #frame_counter < frames_per_cycle*num_seq_reps: #endPoint - posLinear <= dist: #frame_counter <= frames_per_cycle*num_seq_reps: 
        ...
        if acquire_images:
            ...
            if save_images:
                ...
                fdict['sf'] = patch.sf[0]
                fdict['ori'] = patch.ori
                fdict['condName'] = condname #cond_label[condnum-1]#condLabels[int(curr_cond)] #condLabel[int(condType)-1]
