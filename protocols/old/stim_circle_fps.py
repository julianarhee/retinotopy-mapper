...
def save_images_to_disk():
    ...
    while currdict is not None:
        ...
        if save_as_png:
            ...
            fname = '%s/%s/%i_%i_M%i_C%i_SZ%s_SF%s_TF%s_pos%s_cyc%s_stim%s.tif' % (output_path, currdict['condName'], int(currdict['time']), int(currdict['cycle']), int(currdict['frame']), int(currdict['frame_counter']), str(currdict['size']), str(currdict['sf']), str(currdict['tf']), str(currdict['pos']), str(currdict['cycleidx']), str(currdict['stim']))
            tiff = TIFF.open(fname, mode='w')
            tiff.write_image(currdict['im'])
            tiff.close()
