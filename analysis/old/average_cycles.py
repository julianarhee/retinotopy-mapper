...
parser.add_option('--path', action="store",
                  dest="session_path", default="", help="session dir")
...
def average_movies_by_run(run_path, altpath=altpath):
    cond = os.path.split(run_path)[1]
    processed_dir = os.path.join(os.path.split(run_path)[0], 'movies', 'processed_%s_reduce%s_%s_%s%s%s%s%s' % (cond, str(reduce_factor[0]), append_to_name, movie_type, detrend_flag, smooth_flag, mean_sub_flag, mean_flag))
    if not os.path.exists(processed_dir):
        os.makedirs(processed_dir)
    ...
    if 'tif' in im_format:
        ...
        sample = imread(os.path.join(run_path, files[0]))
    ...
    if motion_corrected is True:
        session_dir = os.path.split(run_path)[0]
        curr_cond = os.path.split(run_path)[1]
        ...
        for i, f in enumerate(files):
            ...
            if 'tif' in im_format:
                ...
                im = imread(os.path.join(run_path, f))
...
for run in run_list:
    Process(target=average_movies_by_run, args=(os.path.join(session_path,run), )).start()
