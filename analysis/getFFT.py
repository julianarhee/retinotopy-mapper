...
fext = 'magmaps_%s_%s.pkl' % (cond, str(reduce_factor))
fname = os.path.join(outdir, fext)
with open(fname, 'wb') as f:
    pkl.dump(mag_map, f, protocol=pkl.HIGHEST_PROTOCOL) #protocol=pkl.HIGHEST_PROTOCOL)

fext = 'phasemaps_%s_%s.pkl' % (cond, str(reduce_factor))
fname = os.path.join(outdir, fext)
with open(fname, 'wb') as f:
    pkl.dump(phase_map, f, protocol=pkl.HIGHEST_PROTOCOL) #protocol=pkl.HIGHEST_PROTOCOL)

fext = 'freqs_%s_%s.pkl' % (cond, str(reduce_factor))
fname = os.path.join(outdir, fext)
with open(fname, 'wb') as f:
    pkl.dump(freqs, f, protocol=pkl.HIGHEST_PROTOCOL) #protocol=pkl.HIGHEST_PROTOCOL)
